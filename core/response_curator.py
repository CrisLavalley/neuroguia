...
class ResponseCurator:
    ...
    def _apply_final_control(
        ...
        if turn_intent != "other" and not self._responds_to_turn_intent(
            text=text,
            turn_intent=turn_intent,
            normalized_message=normalized_message,
        ):
            notes.append(f"final_control=priority_{turn_intent}")
        ...
        if self._contains_disallowed_nonanswer_phrase(text) and not self._responds_to_turn_intent(
            text=text,
            turn_intent=turn_intent,
            normalized_message=normalized_message,
        ):
            notes.append("final_control=nonanswer_phrase")
            text = self._build_priority_response(
                turn_intent=turn_intent,
                normalized_message=normalized_message,
                domain=domain,
        ...
        if self._contains_abstract_language(text):
        ...
        if self._is_similar_to_recent_responses(text, chat_history):
    ...
    def _score_quality(
        ...
        if any(token in text_norm for token in ["no tienes que", "no necesitas decidirlo", "no hace falta decidir"]):
            score += 0.06
        if any(token in text_norm for token in ["estoy contigo", "estoy aqui", "vamos despacio", "no tienes que resolver todo ahora"]):
            score += 0.08
